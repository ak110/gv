...
- コードを書いた後は必ず整形およびテストコードの実行をする
  ...
  - ドキュメント（`.md`ファイル）を変更した場合は`pnpm run lint`を実行してtextlint/markdownlint/prettierのチェックを通すこと（`node_modules`がなければ`pnpm install`を先に実行）
  - 全チェックの一括実行: `powershell -File scripts/lint-all.ps1`
